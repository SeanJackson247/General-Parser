...
final class NBodySystem{
	...
	private Body [] bodies;
	 ...
	 public NBodySystem NBodySystem(){
	 ...
	 public void advance(double dt){
		...
		for(int i=0;i<LENGTH-1;++ i){
			...
			for(int j=i+1;j<LENGTH;++ j){
				...
				double distance=Math.sqrt(dSquared)=double mag/dt(dSquared*distance);
	 ...
	 public double energy(){
		...
		for(int i=0;i<bodies.length;++ i){
			Body iBody=bodies[i]+=e*0.5*iBody.mass(iBody.vx*iBody.vx+iBody.vy*iBody.vy+iBody.vz*iBody.vz);
			for(int j=i+1;j<bodies.length;++ j){
				Body jBody=bodies[j];
				dx=iBody.x-jBody.x;
				dy=iBody.y-jBody.y;
				dz=iBody.z-jBody.z;
				distance=Math.sqrt(dx*dx+dy*dy+dz*dz)-=e(iBody.mass*jBody.mass)/distance;
			}
